...
def tensorize_sample_view(result: Mapping[str, Any]) -> dict[str, torch.Tensor]:
    """Convert a ``train_sample`` result into tensors used by the training loop."""
    ...
    source_patch_mm = torch.tensor(float(result["source_patch_mm"]), dtype=torch.float32)
...
def compute_pair_targets(
    ...
    """Compute pair labels used by the pair-relation head."""
...
def build_dataset_item(
    *,
    ...
    return {
        ...
        "source_patch_mm": view_a["source_patch_mm"],
        ...
        "source_patch_mm_a": view_a["source_patch_mm"],
        ...
        "source_patch_mm_b": view_b["source_patch_mm"],
