...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Requested study4 samples before the 2 GB cache budget is applied.",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def _progress_line(event: dict[str, object]) -> str:
    ...
    if stage == "sampling":
        ...
        target = int(event.get("target_examples", event.get("target_studies_budgeted", 0)))
        return f"[sampling:{status}] accepted={accepted}/{target} visited={visited}/{total}"
    if stage == "writing":
        shard_index = int(event.get("shard_index", 0))
        studies_written = int(event.get("studies_written", 0))
        total_studies = int(event.get("total_studies", 0))
        bytes_written = int(event.get("bytes_written", 0))
        return (
            f"[writing:{status}] shard={shard_index} studies={studies_written}/{total_studies} "
        ...
        json.dumps(
            {
                "status": "built",
                **summary,
                "loaded_studies": int(loaded["patches_views"].shape[0]),
